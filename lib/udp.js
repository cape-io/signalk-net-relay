...
const { openSocket } = require('./udp-socket')
...
const ensureSocketClosed = app => new Promise((resolve) => {
  const socket = app.ref.get('socket')
  if (socket) return socket.close(() => { resolve() })
  return resolve()
})
...
function startUdp(app, props) {
  ...
  openSocket(app, { broadcast: hasBroadcastProp(props) })
