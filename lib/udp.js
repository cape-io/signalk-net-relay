...
function createSocket(app, state, { broadcast }) {
  try {
...
const sendData = (app, state, { port, host }) => (data) => {
  // Does it need a try catch if it has an error handler function?
  state.get('socket').send(data, 0, data.length, port, host, (err) => {
